...
public class NotificationsActivity extends Activity implements ActionBar.TabListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		API.getTagsList(Constants.NOTIFICATIONS_CATEGORY_POSITION, new JsonHttpResponseHandler() {
		...
		categoriesList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				API.getNotificationsSearchList(5, 0, categories.get(position).tag, new JsonHttpResponseHandler() {

					@Override
                    public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                        try {
                            AppArmeniaManager.getInstance().resetNotificationsData();
		...
		itemsList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				Intent notificationsDetailsActivity = new Intent(NotificationsActivity.this, NotificationDetailsActivity.class);
				notificationsDetailsActivity.putExtra(HomeActivity.POSITION, NotificationsActivity.this.position);
				startActivity(notificationsDetailsActivity);
			}
		});
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.action_settings:
			Intent appCategoriesActivity = new Intent(NotificationsActivity.this, SettingsActivity.class);
	...
	private void getItemsListByType(String type) {
		ArrayList<AppCategoryItemData> children = AppArmeniaManager.getInstance().notificationsData.get(type);
